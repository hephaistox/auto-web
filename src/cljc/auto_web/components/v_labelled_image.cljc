(ns auto-web.components.v-labelled-image
...
(defn- v-labelled-image
  ...
  * `class` is a string that can be added to the image's class. "
  ...
  (let [href-link (if (map? href-link) href-link {})
        width (wsizes/predefined-size width-kw)
        min-width (wsizes/min-size width-kw)]
    [wlinks/a
     href-link
     [:div.w3-center
      [:img.w3-image
       (assoc (wlinks/img-meta img-link)
              :class class
...
(defn icon-image
  ...
  [img-link href-link]
  (let [href-link (if (map? href-link) href-link {})
        width (wsizes/predefined-size :icon)]
    [wlinks/a
     href-link
     [:img.w3-image
      (assoc (wlinks/img-meta img-link)
